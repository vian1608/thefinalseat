...
async function findBookings(query) {
  ...
  const exact = await supabase
    ...
    .eq('confirmation_code', clean.toUpperCase())
    .limit(20);
...
export const bookingCurrentSearchController = {
  search: async (req, res, next) => {
    try {
      ...
      if (!bookings.length) return res.json({ success: true, data: [], count: 0 });
      ...
      const data = bookings.map(booking => {
        ...
        const departureDate = first.departure_date || null;
